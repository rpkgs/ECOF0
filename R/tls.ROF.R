...
tls.ROF <- function(Y, X, noise1, noise2, nsig, nsim.CI = 1000, nsim.rcc = 1000, REG = TRUE, df2 = NULL, plev = .9, rcc.flg = TRUE, seed=NULL) {
  ...
  if (rcc.flg) {
    ...
    rcc <- Tresi.simu.TLS(Y, X, Ca, nsim.rcc, nn1, nn2, nsig, seed=seed)
  }
  listk(beta = beta0, r1.stat, r2.stat, n, 
    beta.CI = cbind(betalow, beta0, betaup), rcc)
}
